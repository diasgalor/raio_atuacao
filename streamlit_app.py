...
if uploaded_kml and uploaded_table:
    try:
        ...
        # --- FILTROS LATERAIS ---
        gestores_disponiveis = sorted(df_agrupado['GESTOR'].dropna().unique())
        ...
        especialistas_disponiveis = sorted(df_gestor['ESPECIALISTA'].dropna().unique())
        especialistas_selecionados = st.sidebar.multiselect(
            "Selecione Especialista(s) (ou deixe vazio para todos)",
            especialistas_disponiveis,
            default=especialistas_disponiveis
        )
        ...
        # Mostrar número de especialistas selecionados
        st.markdown(f"**Número de Especialistas Selecionados:** {len(df_gestor)}")

        # Criar abas para cada especialista
        ...
        for tab, (_, row) in zip(tabs, df_gestor.iterrows()):
            with tab:
                ...
                # Adicionar linhas e círculos no mapa para o especialista
