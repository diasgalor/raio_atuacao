...
st.markdown("Selecione um gestor, especialista ou unidade (ou 'Todos') para visualizar as unidades atendidas, distâncias e o raio de atuação no mapa.")
...
if kml_file and xlsx_file:
    try:
        ...
        # Interface: seleção por gestor, especialista e unidade
        ...
        with col2:
            st.markdown("<div style='height: 24px;'></div>", unsafe_allow_html=True)  # Alinhamento
        ...
        with col3:
            st.markdown("<div style='height: 24px;'></div>", unsafe_allow_html=True)  # Alinhamento
            unidades_filtradas = set()
            for unidades in resultados[resultados['GESTOR'] == gestor_selecionado]['UNIDADES_ATENDIDAS']:
                unidades_filtradas.update(unidades)
            nomes_unidades = ['Todas'] + sorted(unidades_filtradas)
            unidade_selecionada = st.selectbox("Unidade", options=nomes_unidades, format_func=lambda x: x.title())

        # Exibir card do especialista ou unidade
        if kml_file and xlsx_file:
            if unidade_selecionada != 'Todas':
                # Filtrar por unidade específica
                df_final = resultados[resultados['UNIDADES_ATENDIDAS'].apply(lambda x: unidade_selecionada in x)]
                if not df_final.empty:
                    row = df_final.iloc[0].to_dict()
                    row['UNIDADES_ATENDIDAS'] = [unidade_selecionada]
                    row['DETALHES'] = [(row['ESPECIALISTA'], unidade, dist) for unidade, dist in row['DETALHES'] if unidade == unidade_selecionada]
                    row['DIST_MEDIA'] = row['DETALHES'][0][2] if row['DETALHES'] else 0
                    row['DIST_MAX'] = row['DIST_MEDIA']
                    row['GEOMETRIES'] = [geom for geom in row['GEOMETRIES'] if geom in gdf_kml[gdf_kml['UNIDADE_normalized'] == unidade_selecionada]['geometry'].values]
                    consolidated_data = row
                else:
                    consolidated_data = None
            elif especialista_selecionado == 'Todos':
